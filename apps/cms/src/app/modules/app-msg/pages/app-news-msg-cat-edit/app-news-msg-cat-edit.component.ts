...
import { AppNewsMsgCatDto, AppNewsMsgCatStatuses } from '@core/models';
import { AppMsgApiServ } from '@core/services';

@Component({
  selector: 'cms-app-news-msg-cat-edit',
  templateUrl: './app-news-msg-cat-edit.component.html',
  styleUrls: ['./app-news-msg-cat-edit.component.scss'],
})
export class AppNewsMsgCatEditComponent extends BBDBaseComponent implements OnInit {
  ...
  doDataInit() {
    ...
    // add
    if (!this._id) {
      this.editDto = new AppNewsMsgCatDto();
      this.editDto.status = +AppNewsMsgCatStatuses.啟用;
      this.doFormPatchValue();
      return;
    }
    // update
    this.appMsgApiServ.getAppNewsMsgCatDtoById(this._id).subscribe({
      next: (res) => {
        if (!res) {
  ...
  onSubmit(): void {
    ...
    this.appMsgApiServ.setAppNewsMsgCatDto(this.editDto).subscribe({
      next: (res) => {
        if (!res) {
